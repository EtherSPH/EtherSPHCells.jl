...
@inline function momentum!(p::Particle, q::Particle, r::Float64)::Nothing
    ...
    elseif p.type_ == FLUID_TAG && q.type_ == INFLOW_TAG
        ...
        mean_gap = (p.gap_ + q.gap_) / 2
        reference_kernel_value = kernelValue(mean_gap, smooth_kernel)
        # ! here, use libMirrorPressureForce will cause lack of pressure support in the inflow region
        # ! use Common Pressure Force instead can stablize the simulation, although it's not accurate
        libPressureForce!(
            p,
            q,
            r;
...
vtp_io.file_name_ = "cylinder_2d_re_2000_"
vtp_io.output_path_ = "example/results/cylinder/2d_re_2000"
vtp_io
